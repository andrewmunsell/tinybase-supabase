...
const permanentPostgresErrorCodes = new Set([
...
export const createSupabasePersister = async (
	...
	const createOperations = (content: Content): PendingOperation[] => {
		...
		for (const [tableId, tableConfig] of Object.entries(tableConfigs)) {
			...
			for (const rowId of rowIds) {
				...
				if (next) {
					const currentRow = store.getRow(tableId, rowId);
					operations.push({
						id: operationId(tableId, rowId),
						kind: 'upsert',
						payload: toRemote(tableConfig, rowId, currentRow),
						rowId,
						tableId,
					});
				} else if (previous) {
	...
	const syncNow = (): Promise<void> => {
		...
		syncPromise = (async () => {
			while (syncRequested && !isDestroyed) {
				syncRequested = false;
